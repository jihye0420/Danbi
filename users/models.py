...
class UserManager(BaseUserManager):
    def create_user(self, email, username, team, password=None):
        if email is None:
            raise ValueError('이메일은 필수 입력 사항입니다.')
        if username is None:
            raise ValueError('이름은 필수 입력 사항입니다.')
        if team is None:
            raise ValueError('소속팀은 필수 입력 사항입니다.')
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    team = models.ForeignKey(Team, on_delete=models.CASCADE, related_name='user_team', verbose_name='소속팀')
